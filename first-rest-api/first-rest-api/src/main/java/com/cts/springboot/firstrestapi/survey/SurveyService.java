...
public class SurveyService {
	...
	static // static block initializes the service
	{
	...
	public Question retrieveSpecificSurveyQuestion(String surveyId, String questionId) {
		...
		if (surveyQuestions == null)
			return null;
		Optional<Question> optinalQuestion = surveyQuestions.stream()
				.filter(q -> q.getId().equalsIgnoreCase(questionId)).findFirst();
		if (optinalQuestion.isEmpty())
			return null;
		return optinalQuestion.get();
	}
...
/*
 * Service has all the business logic. static block initializes the service
 * 
 * Optional: A container value which may or may not contain a optional value
 * 
 * Here we get the response code as 200, i.e., status=success it we return
 * success for even null value, which isn't correct.
 * 
 * SecureRandom: A cryptographically strong random number minimally complies
 * with thestatistical random number generator tests
 */
